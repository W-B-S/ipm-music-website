$(document).ready(function(){
	...
	$('.close-btn, #mask').on('click', function(){
		...
		$('#mask, #player-link, #register, #email-address, #wechat-address, #result').css('display', 'none')
	})
    ...
    $(".wechat-result").click(function () {
        $('#result-xcx').css('display', 'none')
        $('#result').css('display', 'block')
        $('#progress-line-box').removeClass('active')
        $("#progress-line-box i:last").removeClass('active')
    ...
    $('#next-btn').click(function () {
        if(!(/^1[3456789][0-9]\d{8}$/.test($("#mobile").val()))){
            alert("电话号码不正确");
            return;
        }
        $.ajax({
            type:'post',
            url : 'https://towerchain.com/TCWarrant/webapi/merchants/generateRCode',
            data:{"merchantsFlag":"TC85450548716761088","identifying": $("#mobile").val()},	//merchantsFlag 我们平台分配的    identifying 用户领取资产的标识(手机号或者邮箱)
            success : function(rsl) {
                if (!rsl || rsl.indexOf("errorMsg") > -1) {
                    alert("抱歉，该号码未预约成功");
                    return;
                }
                $('#valid-mobile, #appoint-result').css('display', 'none')
                $('#result-xcx').css('display', 'block')
                $('#progress-line-box').addClass('active')
                $("#progress-line-box i:last").addClass('active')
                var img = document.getElementById("result-xcx-img");
                img.src='data:image/png;base64,' + rsl;
            }
        });
